...
class RedBlackTree:
    ...
    def search_interval_intersections(self, root, segment):
        ...
        """
        if root and not (root.interval == (0,0)):  # if the root is root Null
            if self.is_intersecting(root, segment):
    ...
    def is_intersecting(self, root, segment):
        """
        ...
        """
        interval_left = root.interval
        interval_right = segment.vert_segment
        if (interval_left[0] > interval_right[1]) or (interval_left[1] < interval_right[0]):  # no overlapping at all
            return False
        elif (interval_left[0] < interval_right[0]) and (interval_left[1] > interval_right[1]): # a rectangle inside a rectangle -> not intersection
            if root.rectangle.x_right > segment.rectangle.x_right:
                return False
        return True
